...
class WideResNet(nn.Module):
    ...
    def forward(self, x, return_features=False):
        ...
        self.before_head_data = features
...
class WideResNet_ASH(nn.Module):
    def __init__(self, depth, num_classes, widen_factor=1, dropout_rate=0.0):
        super(WideResNet_ASH, self).__init__()
        nChannels = [16, 16*widen_factor, 32*widen_factor, 64*widen_factor]
        assert (depth - 4) % 6 == 0, 'depth should be 6n+4'
        n = (depth - 4) // 6
        block = BasicBlock
...
KNOWN_MODELS = OrderedDict([
    ('wrn_40_2', lambda *a, **kw: WideResNet(depth=40, widen_factor=2, *a, **kw)),
    ('wrn_40_1', lambda *a, **kw: WideResNet(depth=40, widen_factor=1, *a, **kw)),
    ('wrn_16_2', lambda *a, **kw: WideResNet(depth=16, widen_factor=2, *a, **kw)),
    ('wrn_16_1', lambda *a, **kw: WideResNet(depth=16, widen_factor=1, *a, **kw)),
    
    ('wrn_40_2_ash', lambda *a, **kw: WideResNet_ASH(depth=40, widen_factor=2, *a, **kw)),
    ('wrn_40_1_ash', lambda *a, **kw: WideResNet_ASH(depth=40, widen_factor=1, *a, **kw)),
    ('wrn_16_2_ash', lambda *a, **kw: WideResNet_ASH(depth=16, widen_factor=2, *a, **kw)),
    ('wrn_16_1_ash', lambda *a, **kw: WideResNet_ASH(depth=16, widen_factor=1, *a, **kw)),
])